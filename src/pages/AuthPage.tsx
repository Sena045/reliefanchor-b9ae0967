      ...
      <Card className="w-full max-w-md shadow-xl border-primary/20">
        ...
        <CardContent>
          {showReset ? (
            ...
            <>
              {/* Google OAuth Button - hide on native app */}
              {!isNative && (
                <>
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full h-12 bg-white hover:bg-gray-50 text-gray-700 border-gray-300 shadow-sm font-medium"
                    onClick={async () => {
                      setLoading(true);
                      try {
                        // Store referral code before OAuth so AppContext can apply it
                        if (!isLogin && referralCode) {
                          const normalized = referralCode.trim().toUpperCase();
                          const isValid = /^[A-Z0-9]{8}$/.test(normalized);
                          if (isValid) {
                  ...
                  <div className="relative my-6">
                    ...
                    <div className="relative flex justify-center text-xs">
                      <span className="bg-card px-3 text-muted-foreground">Or use email and password</span>
                    </div>
                  </div>
                </>
              )}
